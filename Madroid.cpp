...
OSDefineMetaClassAndStructors(Madroid, IOEthernetController);
OSDefineMetaClassAndStructors(MadroidUSBInterface, Madroid);
OSDefineMetaClassAndStructors(MadroidInterface, IOEthernetInterface);

bool Madroid::init(OSDictionary *properties) {
	int i;

	LOG(V_NOTE, "Madroid tethering driver for Mac OS X, by Sudheesh Singanamalla");
...
bool MadroidUSBInterface::start(IOService *provider) {
	IOUSBInterface *intf;
	...
	return Madroid::start(provider);
}

bool Madroid::start(IOService *provider) {
	LOG(V_DEBUG, "start");
...
void Madroid::stop(IOService *provider) {
	LOG(V_DEBUG, "stop");
 ...
 * otherwise, we can have not just reentrant calls to Madroid::enable(), but
...
IOWorkLoop *Madroid::getWorkLoop() const {
	return workloop;
}


bool Madroid::openInterfaces() {
...
bool MadroidInterface::init(IONetworkController * controller, int mtu) {
	maxmtu = mtu;
...
bool MadroidInterface::setMaxTransferUnit(UInt32 mtu) {
	if (mtu > maxmtu) {
...
IONetworkInterface *Madroid::createInterface() {
	MadroidInterface * netif = new MadroidInterface;
...
bool Madroid::createNetworkInterface() {
...
IOReturn Madroid::enable(IONetworkInterface *netif) {
...
IOReturn Madroid::disable(IONetworkInterface * netif) {
...
bool Madroid::createMediumTables() {
	IONetworkMedium	*medium;
...
bool Madroid::allocateResources() {
	int i;
...
IOOutputQueue* Madroid::createOutputQueue() {
	return IOBasicOutputQueue::withTarget(this, TRANSMIT_QUEUE_SIZE);
}

bool Madroid::configureInterface(IONetworkInterface *netif) {
	IONetworkData *nd;
...
IOReturn Madroid::getPacketFilters(const OSSymbol *group, UInt32 *filters) const {
	IOReturn	rtn = kIOReturnSuccess;
...
IOReturn Madroid::getMaxPacketSize(UInt32 * maxSize) const {
	*maxSize = mtu;
	return kIOReturnSuccess;
}

IOReturn Madroid::selectMedium(const IONetworkMedium *medium) {
	setSelectedMedium(medium);
	
	return kIOReturnSuccess;
}

IOReturn Madroid::getHardwareAddress(IOEthernetAddress *ea) {
	UInt32	  i;
	void *buf;
	unsigned char *bp;
	int rlen = -1;
...
IOReturn Madroid::setPromiscuousMode(bool active) {
	(void) active;
...
IOReturn Madroid::message(UInt32 type, IOService *provider, void *argument) {
	IOReturn	ior;
...
UInt32 Madroid::outputPacket(mbuf_t packet, void *param) {
	mbuf_t m;
	size_t pktlen = 0;
...
void Madroid::dataWriteComplete(void *obj, void *param, IOReturn rc, UInt32 remaining) {
	Madroid	*me = (Madroid *)obj;
...
IOReturn Madroid::clearPipeStall(IOUSBPipe *thePipe) {
	IOReturn rc;
...
void Madroid::dataReadComplete(void *obj, void *param, IOReturn rc, UInt32 remaining) {
	Madroid	*me = (Madroid *)obj;
	IOReturn ior;
...
void Madroid::receivePacket(void *packet, UInt32 size) {
	mbuf_t m;
	UInt32 submit;
	IOReturn rv;
...
int Madroid::rndisCommand(struct rndis_msg_hdr *buf, int buflen) {
	int count;
...
int Madroid::rndisQuery(void *buf, uint32_t oid, uint32_t in_len, void **reply, int *reply_len) {
	int rc;
...
bool Madroid::rndisSetPacketFilter(uint32_t filter) {
	union {
		unsigned char *buf;
		struct rndis_msg_hdr *hdr;
